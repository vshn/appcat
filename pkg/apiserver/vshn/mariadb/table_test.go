...
import (
	...
	vshnv1 "github.com/vshn/appcat/v4/apis/apiserver/v1"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/apimachinery/pkg/runtime"
)

func Test_vshnMariaDBBackupStorage_ConvertToTable(t *testing.T) {
	tests := []struct {
		name         string
		obj          runtime.Object
	...
	}{
		{
			...
			obj:      &vshnv1.VSHNMariaDBBackup{},
			wantRows: 1,
		},
		{
			name: "GivenMiltipleBackups_ThenExpectMultipleRows",
			obj: &vshnv1.VSHNMariaDBBackupList{
				Items: []vshnv1.VSHNMariaDBBackup{
					{},
					{},
					{},
				},
...
func Test_vshnMariaDBBackupStorage_ConvertToTable_noduplicate(t *testing.T) {
	v := &vshnMariaDBBackupStorage{}
	obj := vshnv1.VSHNMariaDBBackupList{
		Items: []vshnv1.VSHNMariaDBBackup{
			{
	...
	foo1, ok := got.Rows[0].Object.Object.(*vshnv1.VSHNMariaDBBackup)
	if assert.True(t, ok, "unexpected type for foo1") {
		assert.Equal(t, "foo1", foo1.Namespace)
	}
	foo2, ok := got.Rows[1].Object.Object.(*vshnv1.VSHNMariaDBBackup)
	if assert.True(t, ok, "unexpected type for foo1") {
		assert.Equal(t, "foo2", foo2.Namespace)
	}
	foo3, ok := got.Rows[2].Object.Object.(*vshnv1.VSHNMariaDBBackup)
	if assert.True(t, ok, "unexpected type for foo1") {
