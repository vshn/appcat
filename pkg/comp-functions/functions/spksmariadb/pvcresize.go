...
var mariadbRelease = "mariadb-chart"
...
func ResizeSpksPVCs(ctx context.Context, comp *spksv1alpha1.CompositeMariaDBInstance, svc *runtime.ServiceRuntime) *xfnproto.Result {
	...
	err = svc.GetDesiredComposedResourceByName(release, mariadbRelease)
	if err != nil {
		return runtime.NewFatalResult(fmt.Errorf("cannot mariadb release from desired iof: %w", err))
	}
...
func addStsObserver(svc *runtime.ServiceRuntime, comp *spksv1alpha1.CompositeMariaDBInstance) error {
...
func needReleasePatch(values map[string]interface{}, stsSize int64) (bool, string, *xfnproto.Result) {
	releaseSizeValue, found, err := unstructured.NestedString(values, "persistence", "size")
	if !found {
...
func addDeletionJob(svc *runtime.ServiceRuntime, comp *spksv1alpha1.CompositeMariaDBInstance, newSize string, releaseName string) error {
