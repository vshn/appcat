package vshnnextcloud

import (
	"context"
	"fmt"

	xfnproto "github.com/crossplane/function-sdk-go/proto/v1"
	v1 "github.com/vshn/appcat/v4/apis/vshn/v1"
	"github.com/vshn/appcat/v4/pkg/comp-functions/functions/common"
	"github.com/vshn/appcat/v4/pkg/comp-functions/runtime"
)

// AddBilling enables billing for this service.
// When billingEnabled is true, event-based billing (BillingService CR) is used.
// Otherwise, legacy Prometheus-based metered billing is used.
func AddBilling(ctx context.Context, comp *v1.VSHNNextcloud, svc *runtime.ServiceRuntime) *xfnproto.Result {

	err := svc.GetObservedComposite(comp)
	if err != nil {
		return runtime.NewFatalResult(fmt.Errorf("can't get composite: %w", err))
	}

	if svc.Config.Data["billingEnabled"] == "true" {
		billingOpts := common.BillingServiceOptions{
			ResourceNameSuffix: "-billing-service",
		}
		if comp.Spec.Parameters.Service.Collabora.Enabled {
			billingOpts.Items = []v1.ItemSpec{
				{
					ProductID: "appcat-vshn-nextcloud-office-besteffort",
					Value:     "1",
				},
			}
		}
		return common.CreateOrUpdateBillingServiceWithOptions(ctx, svc, comp, billingOpts)
	}

	if comp.Spec.Parameters.Service.Collabora.Enabled {
		return common.CreateBillingRecord(ctx, svc, comp, common.ServiceAddOns{
			Name:      "office",
			Instances: 1,
		})
	}
	return common.CreateBillingRecord(ctx, svc, comp)
}
