					...
					Containers: []corev1.Container{
						{
							...
							Command: []string{
								"bash",
								"-cefx",
								fmt.Sprintf("oc exec -i deployments/%s -- /install-collabora.sh \"%s\" \"%s\" \"%s\"", comp.GetName(), svc.Config.Data["isOpenshift"], comp.GetName(), comp.Spec.Parameters.AddOns.Office.FQDN),
							},
						},
					},
