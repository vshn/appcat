...
func AddCollaboraSts(comp *vshnv1.VSHNNextcloud, svc *runtime.ServiceRuntime) error {
	...
	return svc.SetDesiredKubeObject(sts, comp.GetName()+"-collabora-code-deployment")
}
...
func AddCollaboraIngress(comp *vshnv1.VSHNNextcloud, svc *runtime.ServiceRuntime) error {
	...
	if svc.Config.Data["collabora_ingress_annotations"] != "" {

		err := yaml.Unmarshal([]byte(svc.Config.Data["collabora_ingress_annotations"]), annotations)
		if err != nil {
	...
	annotations["route.openshift.io/destination-ca-certificate-secret"] = comp.GetName() + "-collabora-code-ingress-certificate"
