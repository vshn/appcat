...
func longRunningTransactionAlert(service, namespace string) promv1.Rule {
	return promv1.Rule{
		...
		Annotations: map[string]string{
			...
			"runbook_url": "https://kb.vshn.ch/app-catalog/how-tos/appcat/vshn/postgres/PostgreSQLLongRunningTransaction.html",
...
func backendsWaitingAlert(service, namespace string) promv1.Rule {
	return promv1.Rule{
		...
		Annotations: map[string]string{
			"description": "The number of backends waiting on locks on {{ $labels.pod }} in namespace " + namespace + " has been increasing rapidly in the last 5 minutes.\n  Please investigate lock contention in your application.",
			"runbook_url": "https://kb.vshn.ch/app-catalog/how-tos/appcat/vshn/postgres/PostgreSQLBackendsWaiting.html",
			"summary":     "PostgreSQL backends waiting on locks",
		},
		Expr: intstr.IntOrString{
			Type:   intstr.String,
			StrVal: `increase(cnpg_backends_waiting_total{namespace="` + namespace + `"}[5m]) > 300`,
		},
		For: nonsla.FiveMinuteInterval,
		Labels: map[string]string{
			"severity": nonsla.SeverityWarning,
...
func deadlockConflictsAlert(service, namespace string) promv1.Rule {
	return promv1.Rule{
		...
		Annotations: map[string]string{
			...
			"runbook_url": "https://kb.vshn.ch/app-catalog/how-tos/appcat/vshn/postgres/PostgreSQLDeadlockConflicts.html",
			"summary":     "PostgreSQL deadlock conflicts detected",
		},
		Expr: intstr.IntOrString{
			Type:   intstr.String,
			StrVal: `increase(cnpg_pg_stat_database_deadlocks{namespace="` + namespace + `"}[5m]) > 10`,
		},
		For: nonsla.FiveMinuteInterval,
		Labels: map[string]string{
			"severity": nonsla.SeverityWarning,
...
func highConnectionsWarningAlert(service, namespace string) promv1.Rule {
	return promv1.Rule{
		...
		Expr: intstr.IntOrString{
			Type:   intstr.String,
			StrVal: `sum by (pod) (cnpg_backends_total{namespace="` + namespace + `"}) / max by (pod) (cnpg_pg_settings_setting{name="max_connections", namespace="` + namespace + `"}) * 100 > 80`,
		},
		For: nonsla.FiveMinuteInterval,
		Labels: map[string]string{
			"severity": nonsla.SeverityWarning,
...
func lowDiskSpaceWarningAlert(service, namespace string) promv1.Rule {
	return promv1.Rule{
		...
		Expr: intstr.IntOrString{
			Type:   intstr.String,
			StrVal: `max(max by(persistentvolumeclaim) (1 - kubelet_volume_stats_available_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*"} / kubelet_volume_stats_capacity_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*"})) > 0.7 OR max(max by(persistentvolumeclaim) (1 - kubelet_volume_stats_available_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*-wal"} / kubelet_volume_stats_capacity_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*-wal"})) > 0.7 OR max(sum by (namespace,persistentvolumeclaim) (kubelet_volume_stats_used_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*-tbs.*"}) / sum by (namespace,persistentvolumeclaim) (kubelet_volume_stats_capacity_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*-tbs.*"}) * on(namespace, persistentvolumeclaim) group_left(volume) kube_pod_spec_volumes_persistentvolumeclaims_info{namespace="` + namespace + `", pod=~"postgresql-.*"}) > 0.7`,
		},
		For: nonsla.FiveMinuteInterval,
		Labels: map[string]string{
			"severity": nonsla.SeverityWarning,
...
func lowDiskSpaceCriticalAlert(service, namespace string) promv1.Rule {
	return promv1.Rule{
		...
		Expr: intstr.IntOrString{
			Type:   intstr.String,
			StrVal: `max(max by(persistentvolumeclaim) (1 - kubelet_volume_stats_available_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*"} / kubelet_volume_stats_capacity_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*"})) > 0.9 OR max(max by(persistentvolumeclaim) (1 - kubelet_volume_stats_available_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*-wal"} / kubelet_volume_stats_capacity_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*-wal"})) > 0.9 OR max(sum by (namespace,persistentvolumeclaim) (kubelet_volume_stats_used_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*-tbs.*"}) / sum by (namespace,persistentvolumeclaim) (kubelet_volume_stats_capacity_bytes{namespace="` + namespace + `", persistentvolumeclaim=~"postgresql-.*-tbs.*"}) * on(namespace, persistentvolumeclaim) group_left(volume) kube_pod_spec_volumes_persistentvolumeclaims_info{namespace="` + namespace + `", pod=~"postgresql-.*"}) > 0.9`,
		},
		For: nonsla.FiveMinuteInterval,
		Labels: map[string]string{
