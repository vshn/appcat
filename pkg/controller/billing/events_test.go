...
func TestLastSentUnitForProduct(t *testing.T) {
	tests := []struct {
		name           string
		billingService *vshnv1.BillingService
		productID      string
		...
		{
			name: "returns unit from sent created event",
			billingService: &vshnv1.BillingService{
				Status: vshnv1.BillingServiceStatus{
					Events: []vshnv1.BillingEventStatus{
						{
			...
			productID:    "prod-123",
			expectedUnit: "instance",
		},
		{
			name: "returns unit from most recent sent scaled event",
			billingService: &vshnv1.BillingService{
				Status: vshnv1.BillingServiceStatus{
					Events: []vshnv1.BillingEventStatus{
						{
			...
			productID:    "prod-123",
			expectedUnit: "replica",
		},
		{
			name: "returns empty when no sent event exists",
			billingService: &vshnv1.BillingService{
				Status: vshnv1.BillingServiceStatus{
					Events: []vshnv1.BillingEventStatus{
						{
		...
		{
			name: "returns correct unit for specific product with multiple products",
			billingService: &vshnv1.BillingService{
				Status: vshnv1.BillingServiceStatus{
					Events: []vshnv1.BillingEventStatus{
						{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			unit := lastSentUnitForProduct(tt.billingService, tt.productID)
