...
func NewMetricsCollector(client dynamic.Interface, labelMappings map[string]string, extraResources map[string][]string, log logr.Logger) *MetricsCollector {
	...
	if err != nil {
		log.Error(err, "Failed to discover XRDs during initialization")
	} else {
		collector.xrds = discoveredXRDs
		log.Info("Resource discovery complete", "xrds", len(discoveredXRDs), "extraResources", len(extraResources))
	}
...
func (c *MetricsCollector) discoverXRDs(ctx context.Context) (map[string][]string, error) {
	...
	defer func() {
		if r := recover(); r != nil {
			c.log.V(1).Info("Recovered from panic during XRD discovery (likely in test environment)", "panic", r)
		}
	}()
	...
	if err != nil {
		...
		c.log.V(1).Info("Could not list XRDs, Crossplane may not be installed", "error", err.Error())
		return result, nil
	}
	...
	for _, xrd := range xrdList.Items {
		...
		if err != nil || !found {
			c.log.V(1).Info("Skipping XRD without group", "name", xrd.GetName())
			continue
		}
		...
		if err != nil || !found {
			c.log.V(1).Info("Skipping XRD without versions", "name", xrd.GetName())
			continue
		}
		...
		if err != nil || !found {
			c.log.V(1).Info("Skipping XRD without plural name", "name", xrd.GetName())
			continue
		}
		...
		for _, versionRaw := range versionsRaw {
			...
			c.log.V(1).Info("Discovered XRD", "apiVersion", apiVersion, "resource", plural, "name", xrd.GetName())
		}
	}
