...
// SemVerMinorsAndPatches is a VersionComparisonStrategy that will determine if the latest version as long as it adheres to semver
// It will update the minor and patch part of the version, it will not consider the Major version.
// It's able to either ignore the build information appended to the semver version, or not.
func SemVerMinorsAndPatches(ignoreBuild bool) func(results VersionLister, currentTag string) (string, error) {
	return func(results VersionLister, currentTag string) (string, error) {
		currentV, err := semver.ParseTolerant(currentTag)
		if err != nil {
			return "", fmt.Errorf("current version %s of release is not sem ver: %v", currentTag, err)
		...
		for _, res := range results.GetVersions() {
			...
			if v.Major == newV.Major && v.Minor > newV.Minor {
				if ignoreBuild && v.Pre != nil {
					continue
				}
				newV = *v
