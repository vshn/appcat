...
func TestVSHNRedis_PassCerdentials(t *testing.T) {
	...
	r.RedisDialer = func(service, name, namespace, organization string, instances int, opts redis.Options) (*probes.VSHNRedis, error) {
		...
		return fakeRedisDialer(service, name, namespace, organization, 1, redis.Options{
...
func fakeRedisDialer(service, name, namespace, organization string, instances int, opts redis.Options) (*probes.VSHNRedis, error) {
	p := &probes.VSHNRedis{
		Service:      service,
		Name:         name,
		Namespace:    namespace,
